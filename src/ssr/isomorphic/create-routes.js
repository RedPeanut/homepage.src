...
function prefixLink(link) { return link; }
...
module.exports = (files, context) => {
  ...
  const routes = {
    path: '/',
    component: requireComponent(require('components/Layout')),
    childRoutes: [],
    indexRoute: {},
    pages,
    templates,
  ...
  templatesWithoutRoot.forEach(templateFile => {
    ...
    const route = {
      ...
      component: requireComponent(context(`./${templateFile.requirePath}`)),
      childRoutes: [],
      indexRoute: {},
      pages,
      templates,
  ...
  staticFileTypes.forEach(type => {
    try {
      ...
      wrappers[type] = requireComponent(require(`wrappers/${type}`))
    } catch (e) {
  ...
  pages.forEach(p => {
    ...
    if (staticFileTypes.indexOf(page.file.ext) !== -1) {
      ...
      page.data = context(`./${page.requirePath}`);
      // console.log('page.file.ext = ', page.file.ext);
      // console.log('handler = ', handler);
      // console.log('page.data = ', page.data);
    } else if (reactComponentFileTypes.indexOf(page.file.ext) !== -1) {
      handler = requireComponent(context(`./${page.requirePath}`));
